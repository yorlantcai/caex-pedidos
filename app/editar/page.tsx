...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ ...GLASS, borderRadius: "1rem", padding: "1.25rem 1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
      <p style={{ fontSize: "0.625rem", letterSpacing: "0.3em", textTransform: "uppercase", fontWeight: 500, ...GRAD_GOLD }}>{title}</p>
      {children}
    </div>
  );
}
...
function EditarForm() {
  ...
  const [form, setForm] = useState({
    cliente: "", telefono: "", marca: "", referencia: "",
    tipo_entrega: "presencial" as TipoEntrega,
    direccion: "", precio_venta: "", vendedor: "", notas: "",
  });
      ...
      <main style={{ position: "relative", zIndex: 10, maxWidth: 560, margin: "0 auto", padding: "2rem 1rem" }}>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <Section title="Cliente">
            <Field label="Nombre *"><input className="inp" required value={form.cliente} onChange={set("cliente")} /></Field>
            <Field label="Teléfono / WhatsApp"><input className="inp" type="tel" value={form.telefono} onChange={set("telefono")} /></Field>
          </Section>

          <Section title="Producto">
            <Field label="Marca"><input className="inp" value={form.marca} onChange={set("marca")} /></Field>
            <Field label="Referencia / Nombre *">
              <input className="inp" required list="perfumes-list" value={form.referencia} onChange={set("referencia")} />
              <datalist id="perfumes-list">{PERFUMES.map(s => <option key={s} value={s} />)}</datalist>
            </Field>
            <Field label="Precio de venta (COP) *">
              <input className="inp" required type="number" min="0" step="1000" value={form.precio_venta} onChange={set("precio_venta")} />
            </Field>
          </Section>
          ...
          <Section title="Extras">
            <Field label="Vendedor (socio)"><input className="inp" value={form.vendedor} onChange={set("vendedor")} /></Field>
            <Field label="Notas adicionales">
              <textarea className="inp" rows={3} value={form.notas} onChange={set("notas")} style={{ resize: "none" }} />
            </Field>
          </Section>
